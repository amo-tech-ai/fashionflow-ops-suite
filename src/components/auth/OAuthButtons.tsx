...
export function OAuthButtons({ mode }: OAuthButtonsProps) {
  ...
  const handleGoogleAuth = async () => {
    ...
    if (error) {
      toast.error(getOAuthErrorMessage(error.message, "Google"));
  ...
  const handleFacebookAuth = async () => {
    ...
    if (error) {
      toast.error(getOAuthErrorMessage(error.message, "Facebook"));
...
function getOAuthErrorMessage(errorMessage: string, provider: string): string {
  if (errorMessage.includes("provider is not enabled")) {
    return `${provider} login is not configured. Please contact support.`;
  }
